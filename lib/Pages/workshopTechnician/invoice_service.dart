...
class PdfInvoiceService {
  Future<Uint8List> createInvoice(List<Product> soldProducts, det,
      String workshopID, String serialNo, var mileage) async {
    String manu;
    String mak;
//  final FirebaseAuth _auth = FirebaseAuth.instance;
    final firestore = FirebaseFirestore.instance;
    // Step #################### correct
    final serial = await firestore.collection('serial').doc(serialNo).get();
    ...
    final workshopT =
        await firestore.collection('workshopTechnicians').doc(email).get();
    ...
    final List<CustomRow> elements = [
      ...
      for (var product in soldProducts)
        CustomRow(
          ...
          (product.vatInPercent / 100 * product.price * product.amount)
              .toStringAsFixed(2),
        ...
        build: (pw.Context context) {
          return pw.Column(
            children: [
              ...
              itemColumn(elements),
              pw.Row(children: [pw.Text(e), pw.Text("$det")]),
              pw.SizedBox(height: 25),
              pw.Text(" "),
              pw.SizedBox(height: 25),
              pw.Text(" "),
...
// Here it will save a pdf version on the device and on the cloud.
  Future<void> savePdfFile(Appointment app, String mileage, List ser,
      String tot, Uint8List byteList) async {
    final firestore = FirebaseFirestore.instance;
    ...
    try {
      ...
      await firestore.collection('Appointments').doc(app.appointmentID).update(
        {
          'reportURL': url,
          'status': "finished",
          'services': ser,
